...
| AppHost | partial | A0.1--A0.4 and G8--G9.4 are implemented and promoted default-dark; the source-backed G9.3 decision retains Current because all eight deletion conditions were unmet; G10.0 accepts but has not implemented one exact installed canary | implement and retain evidence for the G10 canary without changing normal routes; any broader activation, Current deletion, A0.5 launcher, AppServer runtime, and AppService remain separate future changes | AppHost ARD-003, G9 closure, G10 explicit-canary design, and AppHost scope |
