...
| --- | --- | --- | --- | --- |
| Capability graph | partial | Runtime Profile resolution/binding and selected capability composition exist; no complete top-level plan/finalize/mount/projection owner | explicit dependency planning, transactional incremental finalization, live Mount state and read-only graph projection | Harness capability dependency/mount decision |
...
| Coding LSP | partial | active query, lifecycle, diagnostics and Product binding slices are implemented | accepted complete child-scope requirements/specification/traceability and remaining passive feedback | `coding.lsp` scope |
...
| Architecture documentation | partial | recursive Scope governance, generated facts, canonical entrypoints and initial drift guards are implemented; legacy live documents remain to normalize | consistent status/authority and Current/Target/History separation across every canonical scope | architecture method and documentation gates |
