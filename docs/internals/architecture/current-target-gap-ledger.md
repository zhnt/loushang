...
| Hosting | partial | accepted top-level placement and standard-library H0 Contract Model are implemented; current process mechanics remain Harness-owned | implement bounded process, inherited peer endpoint, atomic child-session, and platform owners, then migrate through compatibility adapters without transferring Harness authority | Hosting ARD-002 and Hosting scope |
