...
| Hosting | partial | H0--H6.5b process, endpoint, child-session, Linux/Windows managed preparation, the default-dark Harness adapter, and PLC9C5 Product/native canaries are implemented with retained platform evidence | retain mechanism/authority separation through G9 and any later installed activation | Hosting ARD-002 and Hosting scope |
| AppHost | partial | A0.1--A0.4 and G8--G10 are implemented; G9.3 retains Current because all eight deletion conditions were unmet, and only the exact default-dark G10 canary reaches a short-lived Hosting child with durable rollback and retained cross-platform evidence | any normal-session or broader activation, Current deletion, A0.5 launcher, AppServer runtime, and composition of the separately implemented G11 AppService remain future changes | AppHost ARD-003, G9 closure, G10 explicit-canary design, and AppHost scope |
| Hosted application | partial | G11 implements strict in-process App Contract and AppService mux/session semantics; G12 explicitly composes the foreground AppHost/Coding path; G13.1--G13.2 implement the strict one-writer store and uncomposed AppService atomic mutation/recovery kernel; no installed route selects it | complete the accepted G13 outer continuity owner, Product composition and restart evidence; AppServer transport/local IPC, daemon/process continuity, multi-client control and external surfaces remain separately gated | G11/G12 implemented designs, accepted partially implemented G13 durable-continuity design, AppServer, AppService and AppHost scopes |
