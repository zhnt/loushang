...
| Hosted application | partial | G11 implements the strict App Contract and AppService mux/session semantics; G12 composes the foreground AppHost/Coding path; G13 implements strict one-writer storage, atomic coordination recovery, lease-last lifecycle, current-generation cwd/user-home resume and fresh Harnesstui reattach; no installed route selects G12/G13 | separately design AppServer transport/local IPC, daemon/process continuity, multi-client control and external surfaces without expanding G13 authority | G11--G13 implemented designs, inventory v6, AppServer, AppService and AppHost scopes |
