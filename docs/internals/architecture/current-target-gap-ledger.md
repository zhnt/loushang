...
| Hosted application | partial | G11 implements a strict in-process App Contract, Product-neutral AppService mux/session semantics, one Coding adapter, and an explicit Harnesstui Hosted Profile; no installed route selects it | foreground AppHost composition, AppServer transport/local IPC, daemon continuity, multi-client control and external surfaces remain separately gated | G11 in-process hosted-application design, AppServer and AppService scopes |
