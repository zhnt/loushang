...
| Hosted application | partial | G11 implements strict in-process App Contract and AppService mux/session semantics; G12 explicitly composes the foreground AppHost/Coding path; no installed route selects it and no application coordination survives process loss | implement the accepted G13 one-writer durable coordination/recovery edge; AppServer transport/local IPC, daemon/process continuity, multi-client control and external surfaces remain separately gated | G11/G12 implemented designs, accepted G13 durable-continuity design, AppServer and AppService scopes |
