...
| Hosting | partial | H0--H6.4 process, endpoint, child-session, Linux/Windows managed preparation, and a default-dark Harness adapter are implemented | retain mechanism/authority separation while PLC9C5 separately proves an explicit Product/native Worker activation and rollback canary | Hosting ARD-002 and Hosting scope |
| AppHost | partial | A0.1--A0.4 are implemented: immutable contracts, admitted pinned catalog/router, canonical process-local live bindings, embedded profile attachments, and an optional contract-only AppServer port binder; all remain explicitly constructed with no production composition | add only separately accepted Product composition, launcher, AppServer runtime, or AppService edges; preserve one catalog/runtime owner and default-dark activation | AppHost ARD-003 and AppHost scope |
