...
| AppHost | partial | A0.1--A0.4 and G8--G10 are implemented; G9.3 retains Current because all eight deletion conditions were unmet, and only the exact default-dark G10 canary reaches a short-lived Hosting child with durable rollback and retained cross-platform evidence | any normal-session or broader activation, Current deletion, A0.5 launcher, AppServer runtime, and AppService remain separate future changes | AppHost ARD-003, G9 closure, G10 explicit-canary design, and AppHost scope |
| Hosted application | missing | AppServer has only the A0.4 structural port bundle; AppService and the Hosted Mux Profile do not exist | G11 accepts a strict in-process App Contract, Product-neutral AppService mux/session semantics, one Product adapter, and an explicit Harnesstui Hosted Profile while excluding transport and daemon authority | G11 in-process hosted-application design and AppServer scope |
