...
| AppHost | partial | A0.1--A0.4 plus G8 are implemented: immutable catalog/router, canonical live bindings, embedded profiles, optional AppServer structural binding, and one uncomposed Coding-owned exact-receipt Product/Worker join | complete G9 owner-retention and operational-closure decisions; add launcher, AppServer runtime, AppService, or installed Product routes only through separate accepted slices | AppHost ARD-003 and AppHost scope |
