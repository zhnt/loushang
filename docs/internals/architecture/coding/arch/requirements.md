...
The current core is accepted as behaviorally implemented when the Arch test
suite covers provider normalization, projection/query behavior, cache
correctness, CLI boundaries, tool authorization and Product activation. New
Target requirements remain partial until linked evidence is added to the
traceability matrix.
