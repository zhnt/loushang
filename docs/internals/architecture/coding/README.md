...
preparation, and evidence-linked architecture/runtime diagnostics. Target
Capability graph planning and complete cross-scope mounting must not be treated
as implemented until their Harness owners land.
...
- the full Capability graph/mount lifecycle target is not yet Current;
- LSP target design and implementation evidence are not yet represented by one
  final traceability matrix;
- Coding Arch has Current implementation but previously lacked a canonical
  nested-scope architecture entrypoint;
