...
| start_turn | 请求启动该轮工作的操作；方法名不定义响应时机，具体以 BC-004 的现有完成语义为准 |
| AppClient | [语义端口](../../../../src/loushang/appserver/client.py)，Rust 将实现它的跨语言等价合同；不是直接导入 Python 对象 |
...
2026-09-09 就绪复核：main 已到 `9bc69361`，G17 提供独立的可选
`SessionDiscoveryClientV1` 和有界 cwd/home 候选发现；使用它须显式采用
`local-detachable-discovery/v1`，不能在旧 G16 profile 下调用。本文 BC-003
关于“无全局历史列表”的约束保留，关于“没有发现端口”的旧基线描述仅适用于
既有 AppClientV1/G16。C1 选择该扩展时须同步更新 GUI-CON-002 与 BC-002。
稳定 execution ID 则仅在 #576 任务分支完成底层可选合同，尚未形成 GUI 可用
的提交/去重/查询协议。核对证据及原生主开发环境安排见
...
flowchart LR
    subgraph H[同一主机、相容的本机身份与文件权限]
        ...
        R -->|G16 本机认证连接 / loushang.app/v1| S[显式启动的 Python 服务进程]
...
| 原生 GUI + 同机 G16 | 显式配置本机记录，连接各机开发后端 | 其他 OS 的原生行为与公开发行安装 |
...
GUI-B2 仅使用 `local-detachable/v1`，不自动回退 G14，不通过隐含 HTTP bridge
调用 Python，不把 SSH 转发当作远程 AppClient 合同。开发者显式启动服务；GUI
...
当前事件没有通用请求到 transcript 的关联 ID；不得仅凭文本相同就合并两次
提交。首期可把待处理发送与权威 transcript 分区显示，具体关联策略须在 C1
和 UI 状态设计中验证，不承诺协议未提供的 exactly-once 语义。
...
GUI 状态名称，不新增 wire enum。版本或 profile 不支持时明确拒绝；可选产品
能力缺失时禁用对应入口并说明原因，不虚构通用能力发现 API。
...
| 查询当前会话 | `snapshot_session` | 需要当前 attachment/generation/member；单个快照不能替代整个 membership 屏障 |
...
`start_turn` 的 Ack 保留调用完成语义，不是接收确认，也不能单独证明业务成功。
等待期间通过当前快照/事件显示运行进度，终态结合服务给出的完成、中断或错误
事实呈现。中断请求返回与 turn 已经停止分别展示；本地超时/取消等待不得伪装
为服务中断。响应丢失时记为结果未知，恢复后重新读取事实，不自动重发 mutation。

现有调用链是 AppClient → AppService → Product Session 的异步 prompt，待该调用
正常结束后返回空 `AckV1`；异常走错误路径。依据
[G16 客户端作用域](../../../../src/loushang/appservice/client_scope.py) 与
[Coding 适配](../../../../src/loushang/coding/appservice_adapter.py)。当前 App Contract
有 `turn_started/turn_completed/turn_interrupted` 事件，但没有可查询的独立 turn
对象或通用 `turn_id`。等待返回期间仍可读取事件、发出中断；“长请求”不等于
“界面阻塞”。G16 已将接收后的任务交由应用持有，连接丢失不会自动取消它。

后续可评估“接收后立即返回稳定轮次标识，终态经事件/查询取得”的合同扩展，
以改善关联和恢复。该扩展必须同时定义轮次查询、启动拒绝、事件顺序、断线
恢复和幂等性；返回一个 ID 本身不保证请求恰好执行一次。新增或版本化合同后
才能采用，不允许 GUI 包装器提前返回并把它冒充现有 `AckV1`。
...
事件经现有 `read_events` 有界批量读取；“事件流”描述连续消费效果，不新增服务
推送协议。读取循环与长操作等待并行，轮询也不能形成无等待的空转。
...
分别追踪 cursor，不能把它当全局顺序。assistant delta 是临时输出，最终消息
提交后清除临时段；新快照替换事实投影，不把旧 delta 再追加一次。

展示应按服务可辨认的类型组织消息、状态与交互，后续再接工具和文件变更条目。
GUI 可以创建本地渲染 key，但不得把它作为服务持久身份或跨重连恢复依据。
稳定轮次/内容 ID、条目引用和工具结果结构归 Product/AppService/AppServer
共同定义，GUI 消费；首期只渲染现有投影，不把通用富条目塞入文本自行解析。
...
| 场景 | 正常时序 | 失败或竞争时必须成立 |
| --- | --- | --- |
| 连接并发送 | 用户连接 → 本机准入/认证 → list/attach → 安装屏障 → 用户提交 → start 与事件读取并行 → 服务终态/响应 | attach 未完成不可发送；发送响应丢失保留未知结果；不自动重试 |
...
| GUI-BV-003 | FR-004、FR-005；NFR-003、NFR-005 | BC-004/006 | B1/B2：流式去重、最终消息替换、长 start 等待时 steer/follow-up/interrupt 可达；响应丢失不重发 |
...
| GUI-BV-009 | NFR-001、NFR-010；CON-002、CON-005 | §4、BC-002/010 | B0/B1 三平台独立开发/构建/最小原生回放；B2 各机 G16；源码/锁同步，凭据/产物隔离；开发构建不冒充发行验收 |
...
| GUI-BG-002 | 完整跨语言 payload、认证与本机记录兼容证据缺失 | Mock 先行；真实接入保持 G16 合同 | AppServer owner 主责、GUI owner 消费：C1 提供合法/非法 fixture、整数范围/bridge 表示与三平台准入证据，再进入 B2 |
...
| GUI-BG-009 | 公共协议缺少稳定轮次/内容条目身份和独立轮次查询 | 首期沿用现有 start 完成响应与事件投影；GUI-FUT-001 为后续候选 | Product/AppService/AppServer owners 定义语义与版本，GUI owner 验证展示：进入扩展前完成标识、事件/快照一致性、接收与完成分离、未知结果恢复设计 |
