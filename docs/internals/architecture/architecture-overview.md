...
- Capability dependencies, binding, Mount lifecycle and graph projection gain
  explicit top-level owners without creating a global mutable service locator;
...
[Harness Current Owner Map](harness/current-owner-map.md) explicitly separates
implemented owners from the accepted Capability graph target.
...
| --- | --- |
| top-level Capability dependency planning, transactional finalization and read-only graph projection | accepted Target; no complete live Capability graph owner yet |
...
| Ontology actions, decisions, write-back and source connectors | semantic fact/projection/query foundation exists; operational action plane remains target work |
