...
The accepted, partially implemented G13 hosted-continuity slice adds an optional
...
activation remain separate Target decisions. G13.1--G13.2 implement the strict
store and uncomposed AppService transaction/recovery kernel; the outer
AppHost/Product lifecycle owner remains Target.
...
| hosted application boundary | G11 implements a strict App Contract, in-process AppService named-mux semantics, one Coding hosted Session adapter and an explicit Harnesstui profile; G12 composes an explicit foreground AppHost/Coding path with canonical cwd/user-home create/resume and ordered shutdown; G13.1--G13.2 implement a strict continuity store plus uncomposed atomic AppService persistence/recovery, while the outer AppHost/Product owner remains pending; IPC, daemon/process continuity and installed activation remain future deltas |
