...
| `continuity.provider_packs` | Harness Continuity composition | optional `ordered`; Process; sealed; Product/OEM | Aggregate Contribution | Each pack contributes one or more continuity providers | Executable: all resolved packs compose in profile order and duplicate provider identities are rejected. Phase 5B adds portable import contracts without opening the slot to Extension input. |
