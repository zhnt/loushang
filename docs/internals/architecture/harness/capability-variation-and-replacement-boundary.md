...
| `continuity.provider_packs` | Harness Continuity composition | optional `ordered`; Process; sealed; Product/OEM/Extension | Aggregate Contribution | Each pack contributes one or more continuity providers | Executable: all resolved packs compose in profile order and duplicate provider identities are rejected; Extension input is accepted only through the exact-instance, trust-revalidated Phase 5B Plugin adapter. |
