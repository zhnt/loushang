...
| removed `coding.mode` | Canonical split complete | `harness.host.rpc` owns the shared JSONL RPC host, standard session-operation handlers, state/model/diagnostic dispatch, task draining, and strict transport projection. `harnesstui.conversation` owns plain/JSON output and Agent presentation binding. `channel` owns framing, correlation, cancellation, delivery, and the optional Work adapter. Coding injects only its Product plan, Work profile, domain vocabulary, renderer, and diagnostics through CLI/UI composition; no mode facade or Pi alias remains. |
