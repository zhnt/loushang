...
- Current implementation status: implemented. The narrow declaration bridge,
  exact owner selection, durable Component Host activation, private process
  generation, Coding Product bridge, generation gate, graceful shutdown, and
  security-revocation ordering are executable and regression-tested.
...
The first implementation keeps the internal definition and finalized-selection
decoder under the Continuity owner. The inert v1 wire record lives in the
owner-neutral Plugin resources layer so semantic fingerprinting and owner
admission use one strict codec; it carries no admission or runtime authority
and is not a general Plugin authoring helper.
The decoder emits `CapabilityComponentCandidate` v2 because the Plugin-only
projection adds declaration/evidence fingerprints and the Product authority
ceiling. Existing first-party and legacy Plugin candidates remain v1, retain
their original wire projection and fingerprint domain, and cannot carry the
v2 fields.
...
The Product composition root explicitly owns one
`ContinuityPluginGenerationAuthority` for its concrete runtime. Its locked
single reservation is acquired before the first construction await and is
released only after complete reverse cleanup. It is neither a module-global
registry nor a caller-selected string lookup, and the Product runtime's sealed
binding prevents creation of a peer authority or Hub.

Continuity defines only the Instance-family and security-retirement authority
ports. Their durable `PluginInstanceRuntimeLedger` adapters live under Plugin
management and are injected at the Product composition root; Continuity core
does not import the Plugin lifecycle storage implementation.
...
Selection handles, generation handles, Plugin provenance, final Plugin
composition, construction, and publication are advanced owner internals, not
root `loushang.harness.continuity` API. Provenance and gated Plugin bindings
are owner-constructed, and the internal final composer rejects ordinary
`BoundContinuityProvider` values even when their metadata looks similar.
...
Publication accepts one typed Plugin-lifecycle retirement authority bound to
the generation's exact Instance-revision member set. Its opaque accepted and
`REVOKING` evidence must come from that same authority and cover that exact
set; untyped callbacks or no-op success values are not retirement evidence.
Graceful and security closes share one serialized publication state machine.
A security request arriving while graceful Hub close awaits upgrades that
close before disposal, and concurrent security requests share the same
accepted and `REVOKING` evidence.

The production Plugin-management adapter first fsyncs an append-only exact-set
acceptance record in `PluginContinuitySecurityRetirementJournal`, then issues
opaque owner evidence. After in-process quiesce it calls the authoritative
Instance ledger for every exact revocation and issues `REVOKING` evidence only
after each returned durable snapshot matches its Instance revision,
Installation, revocation record, and state. Partial progress is idempotent and
retryable.

Acceptance append and every Instance activation/family acquisition share the
Instance management operation lock. The ledger's bound acceptance source
rejects an exact accepted revision before a new family can be returned. On
construction, every Instance runtime requires the one acceptance journal whose
canonical path is deterministically derived from that Instance journal path;
the source cannot be omitted or rebound to another durable identity on restart,
and path aliases normalize before their sidecar lock is derived. On startup,
the production Continuity family authority replays every accepted record
through the Instance Host's acceptance-validating, idempotent
`apply_accepted_security_revocations` seam before attempting acquisition, so a
crash between acceptance fsync and live-process poison cannot reopen execution.
The adapter never calls the ordinary `begin_revoke` Host operation directly.
The family authority also rejects a Package ledger backed by another Instance
journal or an acceptance/Package journal path collision before serving work.
After owner-component disposal, typed `REVOKING` evidence is handed to
`PluginPackageLifecycleLedger.handoff_cleanup_and_release`; that existing
write-ahead seam persists the cleanup/package-retention lease before releasing
the exact `owner_generation` family.
...
Hub close, activation-lease aborts, and generation-gate join share one
quiesce deadline. A timed-out abort remains an owned, joinable task; neither
the Hub nor the generation unregisters it until cleanup succeeds. Coding keeps
its sealed runtime reservation until shutdown or failed-construction cleanup
actually completes, so a retry cannot create a peer generation or second Hub.
Failed reverse cleanup from `prepare` is retained in the same generation-gate
inventory even though no activation lease reached the Hub; shutdown and
security retirement cannot release the generation or Instance family while
that cleanup remains pending. Consume and abort record their intent before the
first await, so exactly one can win publication.
...
The complete Phase requires architecture, security/lifecycle, and product/test
review followed by the same reviewers' post-fix re-review.
