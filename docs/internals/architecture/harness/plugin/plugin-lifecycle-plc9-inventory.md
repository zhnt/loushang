...
| Config persistence and fence | `src/loushang/harness/config/agent/types.py`, `src/loushang/harness/config/agent/_settings_patch.py`, `src/loushang/harness/config/agent/_settings_codec.py`, and `src/loushang/harness/config/agent/manager.py::SettingsManager.bind_plugin_enablement_legacy_mutation_guard` | Defines/decodes the legacy `disabled_plugins` setting; fence-aware mutation APIs reject peer writes after receipt and expose one derived publisher to the bound Product authority | Retain only through the minimum-version/downgrade gate for fence-aware runtimes; pre-fence direct downgrade is unsupported and requires offline restore/roll-forward, then PLC9E deletes the field and mutators after evidence |
