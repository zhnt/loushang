...
| --- | --- | --- | --- |
| Shared CLI mutation | `src/loushang/harness/cli/resource_toggles.py::apply_resource_toggles` | Plugin enable/disable and source add/remove call an injected settings manager directly | PLC9A routes enable/disable through the common management command port and source add/remove through a typed Source Authority application port; Skill settings remain independently owned |
| Shared CLI listing | `src/loushang/harness/cli/plugin_listing.py::list_plugin_records` | Re-reads settings, inspects mutable sources, constructs `PluginResolutionAuthority`, and projects `enabled` | PLC9A replaces this with the common correlated query port; formatting stays a pure adapter |
...
| Coding continuity bootstrap | `src/loushang/coding/continuity_bootstrap.py::bind_coding_configured_continuity` | Reads and fingerprints `disabled_plugins`, constructs `PluginResolutionAuthority`, and also consults durable desired state | Remove the settings veto and fingerprint input after replay-safe desired-state migration; retain exact desired-revision replay |
