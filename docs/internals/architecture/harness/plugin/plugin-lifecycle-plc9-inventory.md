...
| Coding compatibility writer | `src/loushang/coding/plugin_enablement_compatibility.py::CodingPluginEnablementCompatibilityWriter` | A1-3 serializes canonical projection capture under the workspace coordination lock and submits a typed projection through one opaque settings-owner capability | Temporary Product-owned downgrade coordinator; the settings owner atomically reloads/preserves/publishes/verifies, partial publication is repaired, and canonical desired state is never rolled back |
...
| Coding configured Source projection | `src/loushang/coding/plugin_management_cli.py::CodingConfiguredPluginSourceProjection` | A1-3 projects configured Source facts relative to the expanded workspace path | Retain as an inert Product query adapter; it may inspect configured Sources but cannot construct or mutate a durable lifecycle ledger |
| Coding management CLI composition | `src/loushang/coding/plugin_management_cli.py::build_coding_plugin_management_cli_binding` | A1-3 composes common command/query ports and invokes the Product-owned compatibility reconciler before returning the transport binding | Retain as Product composition; command admission requires a common projected migration receipt |
...
| Config persistence and fence | `src/loushang/harness/config/engine.py::LayeredConfig.transaction`, `src/loushang/harness/config/runtime.py::ScopedConfigRuntime.transaction`, `src/loushang/harness/config/_file_transaction.py`, `src/loushang/harness/config/agent/types.py`, `src/loushang/harness/config/agent/_settings_patch.py`, `src/loushang/harness/config/agent/_settings_codec.py`, and `src/loushang/harness/config/agent/manager.py::SettingsManager.bind_plugin_enablement_legacy_mutation_guard` | `LayeredConfig` serializes every path-backed persistent write by normalized path and performs the strict refresh; `ScopedConfigRuntime` projects one revisioned change and both owners notify only after mutation locks are released. The settings schema defines/decodes the legacy `disabled_plugins` field, retains identity-keyed guards and latest projections for every workspace sharing the settings owner, aggregates a conservative downgrade view, and rejects fenced peer writes after any receipt | Retain the generic config transaction; retain the legacy field/fence only through the minimum-version downgrade gate, then PLC9E deletes the field and mutators after evidence |
