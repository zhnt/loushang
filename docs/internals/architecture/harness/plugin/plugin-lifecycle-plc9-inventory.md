...
| Coding compatibility writer | `src/loushang/coding/plugin_enablement_compatibility.py::CodingPluginEnablementCompatibilityWriter` | A1-3 captures canonical journals and the existing coordination lock relative to one pinned no-follow private-root handle, then submits a monotonic typed projection through the single Product registry authority | Temporary Product-owned downgrade coordinator; the registry aggregates workspace projections without holding its lock across config I/O, the settings owner atomically reloads/preserves/publishes/verifies, partial publication is repaired, and canonical desired state is never rolled back |
