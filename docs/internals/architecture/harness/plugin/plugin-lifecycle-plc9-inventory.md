...
| Config persistence and fence | `src/loushang/harness/config/runtime.py::ScopedConfigRuntime.transaction`, `src/loushang/harness/config/_file_transaction.py`, `src/loushang/harness/config/agent/types.py`, `src/loushang/harness/config/agent/_settings_patch.py`, `src/loushang/harness/config/agent/_settings_codec.py`, and `src/loushang/harness/config/agent/manager.py::SettingsManager.bind_plugin_enablement_legacy_mutation_guard` | Serializes every persistent settings write by normalized path with strict refresh and post-unlock notification; defines/decodes the legacy `disabled_plugins` field, rejects fenced peer writes after receipt, and exposes one derived publisher to the bound Product authority | Retain the generic config transaction; retain the legacy field/fence only through the minimum-version downgrade gate, then PLC9E deletes the field and mutators after evidence |
