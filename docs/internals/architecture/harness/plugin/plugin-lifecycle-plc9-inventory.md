...
| Coding compatibility writer | `src/loushang/coding/plugin_enablement_compatibility.py::CodingPluginEnablementCompatibilityWriter` | A1-3 serializes canonical projection publication under the workspace coordination lock, retains unmigrated ids, and binds the legacy settings mutation fence | Temporary Product-owned downgrade sink; startup/CLI binding repairs partial publication and canonical desired state is never rolled back |
