...
| Coding B Capability selection | `src/loushang/coding/package_product_capabilities.py` | Checks built-in `manifest.enabled` against the selected Product revision and mount policy | Retain as a selected-revision check; a manifest flag alone cannot replace Product desired-state authority |
