...
The directory name does not disclose the workspace path. The root is created
with private permissions. `plugins.state` is visible in machine-resource
status, but generic cleanup never owns it; only Plugin lifecycle authorities
may retire its records or packages.
...
enters durable state or the workspace.
...
revision handles before returning.
...
pre-session picker ran. `ScreenSurfaceManager` receives a stable reference; it
does not resolve Plugins, recover journals, or construct a Hub synchronously.
...
until Plugin Management has staged and selected that revision; process restart
...
after failure, shutdown/revision cleanup, and stable redacted diagnostics; the
full non-live repository gate must pass.
