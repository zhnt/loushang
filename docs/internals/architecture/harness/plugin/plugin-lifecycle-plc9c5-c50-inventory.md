...
- Observation base: `5759c1bf` plus the C5.1 candidate
...
transition. Source and executable tests remain higher authority. Names owned by
C5.2--C5.4 remain proposals. A source row records the narrow seam needed to
...
| `C5-CUR-WORKER-PUBLIC` | `src/loushang/harness/worker/__init__.py` | public Worker contracts export launch/session/supervisor, owner selection, H6.4 adapter, C4 query-adapter mechanics, and exactly three C5.1 policy/receipt/authority contracts; the coordinator and native-profile contract are not public | freeze the exact Current `__all__`; C5.2 may add only `ProductWorkerNativeProfilePort`; private coordinator/cleanup/status/bridge/platform types remain unexported |
...
- Product Worker policy/receipt/coordinator contracts and deterministic C5.1
  evidence exist only inside Worker. No eligible native profile supplier,
  native canary report, production allowlist/issuer/state store, Product
  consumer, or cross-entrypoint receipt report exists.
...
| restart settlement | C5.1 fake joins protocol terminal, exact domain retirement, durable complete-tree settlement/debt, host/boot identity, restart budget, and construction-pinned evidence identity; registered commit ambiguity auto-settles on the same boot or requires exact changed-boot recovery proof | C5.2/C5.3 platform settlement evidence and C5.4 Product recovery | generic contract passes with owner-substitution, orphan, compaction, and forged-proof fences; no native tree witness or production recovery route exists |
