...
| --- | --- | --- | --- |
| exact Worker identity and request evidence | Current launch and Supervisor tests | `test_hosting_adapter_maps_worker_and_publishes_atomic_session` | `test_hosting_adapter_preserves_managed_capture_and_worker_semantic_fence` |
| pre-transaction and final semantic validation | Current binding tests | mapping test plus `test_hosting_adapter_rechecks_abort_at_final_pre_spawn_fence` | managed-capture test proves the same two Worker validations surround the delegated caller/native fence |
| aggregate process plus protocol endpoint | Current compatibility aggregate tests | mapping test and `test_supervisor_can_handshake_through_hosting_aggregate` | the same aggregate adapter is used after managed preparation; no second session type exists |
| failure and cancellation ownership | Current launch rollback tests | H5 invalid-session and final-fence rollback tests | `test_hosting_adapter_managed_capture_cancellation_retains_delegate_cleanup` plus H6.1 reservation rollback tests |
