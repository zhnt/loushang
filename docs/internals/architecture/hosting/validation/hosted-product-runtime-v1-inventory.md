...
| optional Linux managed-file owner | `src/loushang/apphost/managed/_files.py` | private canonical directory/regular-file admission, stable nonblocking locks and bounded identity-checked record publication; uncomposed, not a service registry or process owner |
| optional managed name registry | `src/loushang/apphost/managed/_database.py` and `src/loushang/apphost/managed/registry.py` | bounded SQLite name intents and Product/workspace/profile reuse index with exact operation matching; no native liveness authority, process activation or CLI composition |
| optional managed instance journal | `src/loushang/apphost/managed/lifecycle.py` | stable service fence, durable generations and immutable Linux native lookup registration; commit requires matching registration, stop/settlement facts stay separate; stored identity is neither liveness nor permission to terminate |
...
| optional managed child application owner | `src/loushang/apphost/managed/child.py` | staged application lifecycle around durable handoff, one retained control IO worker and independent application cleanup; no Product selection, spawn, CLI activation or process-exit claims |
