...
| optional managed instance journal | `src/loushang/apphost/managed/lifecycle.py` | stable per-service fence and durable instance generations, handoff/stop transitions and matching monotonic settlement observations; neither native process identity nor permission to terminate a process |
...
| optional managed handoff binding | `src/loushang/apphost/managed/handoff.py` | exact instance/attempt journal adapter for Hosting handoff; stale/uncertain reads never grant current authority; no process ownership |
