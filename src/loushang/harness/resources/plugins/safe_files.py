"""Bounded, no-follow regular-file capture below one trusted directory."""
...
def _capture_descriptor_relative(
    ...
    *,
    ...
    try:
        ...
        body = _bounded_read(descriptor, max_bytes=max_bytes, path=display_path)
...
def _capture_portable(
    ...
    *,
    ...
    try:
        ...
        metadata = os.fstat(descriptor)
        after = _portable_path_metadata(root, relative, display_path=display_path)
        if (
...
def _bounded_read(descriptor: int, *, max_bytes: int, path: Path) -> bytes:
    chunks: list[bytes] = []
