...
def apply_transcript_style(
    ...
    *,
    ...
    """Apply semantic theme tokens to a presentation-ready transcript line."""
...
def _flag_spans(line: str) -> list[_Span]:
    return [
        _Span(match.start(1), match.end(1), "transcript.tool.flag")
...
def _meta_spans(line: str) -> list[_Span]:
    ...
    if no_output != -1:
        spans.append(
            _Span(
                no_output,
                no_output + len("(no output)"),
                "transcript.tool.meta",
            )
        )
    spans.extend(
        _Span(match.start(), match.end(), "transcript.tool.meta")
        for match in _ELLIPSIS_LINES_RE.finditer(line)
    )
    spans.extend(
        _Span(match.start(), match.end(), "transcript.tool.meta")
        for match in _COLLAPSED_LINES_RE.finditer(line)
    )
    spans.extend(
        _Span(match.start(), match.end(), "transcript.tool.meta")
        for match in _TIMING_RE.finditer(line)
    )
    spans.extend(
        _Span(match.start(), match.end(), "transcript.tool.meta")
        for match in _GIT_NOOP_RE.finditer(line)
    )
    return spans
...
__all__ = ["apply_transcript_style"]
