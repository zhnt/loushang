...
ARCHITECTURE_ROOT = Path("docs/internals/architecture")
ARCHITECTURE_METHOD_ROOT = Path("docs/internals/architecture-method")
CANONICAL_DOCUMENTS = (
...
def test_canonical_architecture_documents_declare_authority_and_status() -> None:
    missing: list[str] = []
    for path in CANONICAL_DOCUMENTS:
        ...
        for field, values in STATUS_VALUES.items():
            ...
            if not re.search(rf"^- {re.escape(field)}: ({allowed})(?:\s|$)", text, re.M):
                missing.append(
                    f"{path.as_posix()}: {field} must start with one of {values}"
                )

    assert missing == []


def test_canonical_architecture_document_links_are_repository_relative_and_valid() -> (
    None
):
    errors: list[str] = []
    for path in CANONICAL_DOCUMENTS:
        ...
        if "/home/" in text or "C:\\" in text:
            errors.append(f"{path.as_posix()}: contains a machine-local absolute path")
        for raw_target in MARKDOWN_LINK.findall(text):
            ...
            if not resolved.is_file():
                errors.append(f"{path.as_posix()}: missing link target {target}")
